...
이 프로젝트는 학습 목적으로 Redis 서버를 밑바닥부터(from scratch) 구현하는 프로젝트입니다. 
유명한 학습 플랫폼인 **CodeCrafters의 "Build Your Own Redis" 챌린지 커리큘럼**을 차용하여, 로컬 환경에서 TDD(Test-Driven Development) 방식으로 진행합니다.
...
3. **코드 리뷰 교차 검증**:
   ...
   - **에이전트(시니어 리뷰어)**에게 명시적으로 리뷰를 요청하여, 놓친 엣지 케이스, 소켓/스트림 누수, 동시성 이슈 등 아키텍처 관점의 깊이 있는 피드백을 받습니다.
